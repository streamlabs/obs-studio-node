...
std::string censoredJson(obs_data_t *data, bool pretty)
{
	if (!data) {
		return "";
	}
	...
	std::string s = pretty ? obs_data_get_json_pretty(clone) : obs_data_get_json(clone);
...
void censorRecurse(nlohmann::json &data)
{
...
std::string censoredJson(nlohmann::json data, bool pretty)
{
	censorRecurse(data);
