...
static void HandleSampleRate(obs_property_t *prop, const char *id)
{
	...
	const uint32_t sampleRate = config_get_uint(ConfigManager::getInstance().getBasic(), "Audio", "SampleRate");
...
static std::map<std::string, std::vector<int>> encoderBitrates;
...
static std::map<int, std::string> simpleAACBitrateMap;
...
static void PopulateSimpleAACBitrateMap()
{
	...
	std::call_once(once, []() {
		...
		const std::string fallbackEncoder = encoders[0];
...
static std::map<int, std::string> simpleOpusBitrateMap;
...
static int FindClosestAvailableSimpleBitrate(int bitrate, const std::map<int, std::string> &map)
{
	int prev = 0;
